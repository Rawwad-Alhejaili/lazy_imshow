...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 13 03:12:01 2022

I have written this function to make it much quicker to show high quality
figures. I find that showing images using plt.imshow to be very annoying
because the defaults do not suit my needs at all, and adjusting them every
single time does get very annoying very quickly. As such, I decided to write
this function to cut down on the tedium. And surprisingly, this function makes
showing images much more convenient. Below I will write a summary that includes 
    ...
    for ax in grid:
        ...
        if len(Im.shape) == 2 or (len(Im.shape) == 3 and Im.shape[2] == 1): #If the image was grayscale,
            ...
            if Im.shape[-1] == 1: #If the image was grayscale,
                ...
                show = ax.imshow(Im, vmin=minv[idx], vmax=maxv[idx], aspect=aspect, **kwargs)
                # if L == 256 and (minimum >= 0):
                ...
                show = ax.imshow(Im[:,:,0], cmap=cmap, vmin=minv[idx], vmax=maxv[idx], aspect=aspect, **kwargs)
        else:
